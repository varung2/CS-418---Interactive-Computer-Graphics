...
//-------------------------------------------------------------------------
	...
	function setupSphereBuffers() {
			...
			var sphereSoup=[];
			var sphereNormals=[];
			var numT=sphereFromSubdivision(2,sphereSoup,sphereNormals);
...
function draw() { 
		...
		for (var i = 0; i < mySphereStack.length; i++) {
			//push the mv matrix
			mvPushMatrix();

			var s = mySphereStack[i].get_radius();
			var pos = mySphereStack[i].get_position();

			// vec3.set(translateVec,1,-2,-3);					//position of particle
			mat4.translate(mvMatrix, mvMatrix, pos);

			//apply the sphere properties
			vec3.set(transformVec,s,s,s); 			//apply the sphere radius properties
			mat4.scale(mvMatrix, mvMatrix,transformVec); 	//apply to mv matrix

			uploadModelViewMatrixToShader();				//apply the mv to the shader
...
function animate() {
	...
	var g = 0.0; //gravity
	var d = 0.0; //drag force

	//if gravity is enabled set it to the factor
	if (enable_grav == 1) {
		g = -2.0;
	} 

	//if drag is enabled, set to this factor
	if (en_drag == 1) {
		d = 0.99;
	} 

	//if we want to increase the velocity, set it to greater than one
	if (en_force == 1) {
		d = 1.01;
	}	

	//enable realtime calculations
	var t = false;
	if (en_realtime == 1) {
		t = true;
	}

	//check if it is the first animated frame
	if (frame_time == 0) {
		t = false;
		frame_time = Date.now();
	} else {
	...
	//update the sphere positions every frame
	for (var i = 0; i < mySphereStack.length; i++) {
		mySphereStack[i].update_sphere(g, d, t, frame_time);
	}
}
 ...
 */
 function startup() {
	...
	var velocity = vec3.fromValues(-2.0, 2.0, 3.0);
	var pos = vec3.fromValues(0.0, 0.0, -20.0);
	var accel = vec3.fromValues(0.0, 0.0, 0.0);
	var col = vec3.fromValues(0.0, 0.0, 0.0);
	var rad = 2.0*Math.random()+1.0;

	mySphereStack.push(new sphereMesh(velocity, pos, accel, col, rad, time_step));
...
function keypress(evt) {
	...
	if (evt.key == "p" || evt.key == "P") {
		...
		for (var i = 0; i < particles; i++) {
			var rand_radius = 1.0*Math.random();
			var v1 = 3.0*(Math.random()*2.0 - 1.0); var v2 = 3.0*(Math.random()*2.0 - 1.0); var v3 = 3.0*(Math.random()*2.0 - 1.0);
			var pos = vec3.fromValues(0.0, 0.0, -20.0);
			var accel = vec3.fromValues(0.0, 0.0, 0.0);
			var col = vec3.fromValues(0.0, 0.0, 0.0);
			var vel = vec3.fromValues(v1, v2, v3);
			mySphereStack.push(new sphereMesh(vel, pos, accel, col, rand_radius, time_step));
		}

		console.log(mySphereStack.length);
	}

	//enable gravity
	if (evt.key == "g" || evt.key == "G") {
		console.log(evt.key)
		if (enable_grav == 1) {enable_grav = 0;} else {enable_grav = 1;}
	}
	...
	if (evt.key == "d" || evt.key == "D") {
		console.log(evt.key)
		en_force = 0;
		if (en_drag == 1) {en_drag = 0;} else {en_drag = 1;}
	}

	//enable the additive force
	if (evt.key == "f" || evt.key == "F") {
		console.log(evt.key)
		en_drag = 0;
		if (en_force == 1) {en_force = 0;} else {en_force = 1;}
	}

	//remove spheres
	if (evt.key == "r" || evt.key == "R") {
		console.log(evt.key)
		var remove = document.getElementById("inv_part").value;
	...
	if (evt.key == "t" || evt.key == "T") {
		console.log(evt.key)
		if (en_realtime == 1) {en_realtime = 0;} else {en_realtime = 1;}
	}

	//clear the screen by removing all particles
	if (evt.key == "c" || evt.key == "C") {
		console.log(evt.key)
		while (mySphereStack.length > 0) {
			mySphereStack.pop();
		}
	}
}
