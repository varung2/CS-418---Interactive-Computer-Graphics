	...
	loadFromOBJ(fileText)
	{
		...
		for (var i=0; i<lines.length;i++){
			...
			if (tokens[0][0]=="#"){
				...
				var a = parseFloat(tokens[1]);
				var b = parseFloat(tokens[2]);
				var c = parseFloat(tokens[3]);
				this.maxCoord = Math.max(Math.abs(a), Math.abs(b), Math.abs(c));
				this.vBuffer.push(a);
				this.vBuffer.push(b);
				this.vBuffer.push(c);
				this.numVertices++;
		...
		if (this.isObj == true) {
			...
			for (var j = 0; j < this.vBuffer.length; j++) {
				this.vBuffer[j] = this.vBuffer[j]/this.maxCoord;
			}
		}
...
/** Function to return the maximum coordinate (used to generate Cube Mesh)
 */
getMaxVal(){
	return this.maxCoord;
}

/** sets the is object, else is a skybox condition */
setisObj(cond) {
	this.isObj = cond;
}
